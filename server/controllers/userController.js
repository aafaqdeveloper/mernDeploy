...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (user) {
        const { _id, name, email, photo, phone, bio, address, role } = user
        res.status(201).json({
            _id, name, email, photo, phone, bio, address, role, token
        })
    } else {
        res.status(400)
...
const getUser = asyncHandler(async (req, res) => {
    ...
    if (user) {
        const { _id, name, email, photo, phone, bio, address, role } = user
        res.status(201).json({
            _id, name, email, photo, phone, bio, address, role
        })
    } else {
        res.status(400)
...
const updateUser = asyncHandler(async (req, res) => {
    ...
    if (user) {
        const { name, email, photo, phone, bio, address } = user
        user.email = email
...
const forgotPassword = asyncHandler(async (req, res) => {
    ...
    // Construct Reset Url
    const resetUrl = `http://fury.aafaqhassan.com/resetpassword/${resetToken}`
